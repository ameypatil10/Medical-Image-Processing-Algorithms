...
background_data = reshape(imageNoisy(1:50,1:50),50*50,1);
sigma = 1;
sigma_absolute = std(background_data);
sigma_for_real_part = std(real(background_data));
sigma_for_img_part = std(background_data - real(background_data));
...
figure(15);hold on;title('Objective Function for Quadration Function Prior');xlabel('Iterations');ylabel('Cost Function');
...
figure(16);hold on;title('Objective Function for Huber Function Prior');xlabel('Iterations');ylabel('Cost Function');
...
figure(17);hold on;title('Objective Function for 3rd Function Prior');xlabel('Iterations');ylabel('Cost Function');
...
%% Analysis
figure(18);imshow(reshape(imageNoisy,256,256));title('Noisy Image');
figure(19);imshow(reshape(X1,256,256));title('Prior1 Denoised Image');
figure(20);imshow(reshape(X2,256,256));title('Prior2 Denoised Image');
figure(21);imshow(reshape(X3,256,256));title('Prior3 Denoised Image');
